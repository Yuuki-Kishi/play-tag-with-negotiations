...
//                guard let isCatchable = fugitive.isCatchable.first(where: { $0.playerUserId == samePositionChaser.playerUserId })?.isCatchable else { return [] }
        ...
        fugitiveLoop: for fugitive in fugitives {
            ...
            for chaser in chasers {
                ...
                if fugitivePosition == chaserPosition {
                    let isCatchable = fugitive.catchers.contains { $0 == chaser.playerUserId }
                    if isCatchable { continue fugitiveLoop }
                }
